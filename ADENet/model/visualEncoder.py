...
class ResNetLayer(nn.Module):
    ...
    def forward(self, inputBatch):
        ...
        batch = self.relu(self.bn1a(self.conv1a(inputBatch)))
        ...
        batch = self.relu(self.outbna(batch))

        # batch = F.relu(self.bn1b(self.conv1b(batch)))
        batch = self.relu(self.bn1b(self.conv1b(batch)))
        batch = self.conv2b(batch)
        residualBatch = intermediateBatch
        batch = batch + residualBatch
        # outputBatch = F.relu(self.outbnb(batch))
        outputBatch = self.relu(self.outbnb(batch))
        return outputBatch
