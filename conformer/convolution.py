...
class ConformerConvModule(nn.Module):
    ...
    def __init__(
        ...
        self.Swish = Swish()
        self.pointWishConv1d2 = PointwiseConv1d(in_channels, in_channels, stride=1, padding=0, bias=True)
    ...
    def forward(self, inputs, convInfo=None):
        ...
        x = self.transpose(x)
        x = self.pointWishConv1d(x)
        x = self.gru(x)
        x = self.DepthwiseConv1d(x)
        x = self.BatchNorm1d(x)
        x = self.Swish(x)
        x = self.pointWishConv1d2(x)
        x = self.dropout(x)
...
class Conv2dSubampling(nn.Module):
    ...
    def forward(self, inputs: Tensor, input_lengths: Tensor) -> Tuple[Tensor, Tensor]:
        ...
        output_lengths = input_lengths >> 2    # 缩小4倍
        output_lengths -= 1
